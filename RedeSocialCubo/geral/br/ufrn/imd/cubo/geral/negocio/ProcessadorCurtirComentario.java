...
/** 
 * Classe responsável por curtir um comentário de publicação.
 * 
 * @author Renan
 */
public class ProcessadorCurtirComentario extends ProcessadorCadastro {
	...
	protected void iniciarExecucao() throws NegocioException, ArqException {
		...
		try {
			...
			//Verificando se o usuário já curtiu o comentário em questão
			...
			if (ValidatorUtil.isEmpty(curtidaBanco)){
				//O usuário não curtiu o comentário ainda. A ação é para curtir
				Curtida curtida = new Curtida();
				curtida.setComentario(obj);
				dao.create(curtida);
			} else {
				//O usuário já curtiu o comentário. A ação é para desfazer a curtida
				dao.delete(curtidaBanco);
			}
			
			dao.flush();
			
			//Atualizando total de curtidas do comentário...
